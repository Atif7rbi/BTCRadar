...
def create_dashboard(market, signal_service, trade_service, trade_store, cfg: dict, coinalyze_store=None, coinalyze_collector=None, alert_service=None) -> Blueprint:
    ...
    def coinalyze_refresh():
        if not coinalyze_collector:
            return jsonify({'ok': False, 'error': 'Coinalyze collector is not initialized'}), 400

        # Avoid repeated manual refresh calls. The collector already refreshes hourly.
        last_refresh = getattr(coinalyze_collector, 'last_refresh_at', None)
        if last_refresh:
            try:
                last_dt = datetime.fromisoformat(str(last_refresh).replace('Z', '+00:00'))
                age_sec = (_utc_now() - last_dt).total_seconds()
                if age_sec < 300:
                    return jsonify({
                        'ok': True,
                        'cached': True,
                        'message': 'Recent Coinalyze data already exists. Manual refresh skipped to avoid rate limits.',
                        'rows_saved': getattr(coinalyze_collector, 'last_rows_saved', 0),
        ...
        try:
            return jsonify(coinalyze_collector.refresh_once())
    ...
    def close_trade(trade_id: int):
        trade_service.close_trade(trade_id)
        return jsonify({'ok': True})
